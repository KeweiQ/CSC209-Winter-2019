...
int read_from(int client_index, struct sockname *usernames) {
    int read_fd = usernames[client_index].sock_fd;
    char buf1[BUF_SIZE + 1];

    int num_read = read(read_fd, &buf1, BUF_SIZE);
    buf1[num_read] = '\0';
    ...
    for (int index = 0; index < MAX_CONNECTIONS; index++) {
        if (usernames[index].sock_fd > -1) {
            if (num_read == 0 || write(usernames[index].sock_fd, buf2, strlen(buf2)) != strlen(buf2)) {
                usernames[client_index].sock_fd = -1;
            }
        }
    }
