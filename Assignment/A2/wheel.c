...
char **get_word_list_of_length(char **words, int *len) {
    ...
    if (fgets(s, BUF_SIZE, stdin) == NULL) {
        fprintf(stderr, "no value was read!\n");
        exit(1);
    }
    s[strlen(s) - 1] = '\0';
    ...
    char **list = prune_word_list(words, length, &words_remaining);
	...
	free(list);
        length = strtol(s, NULL, 10);
        words_remaining = 0;
        list = prune_word_list(words, length, &words_remaining);
    }
